...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler(REPORTS_DIR / "pipeline.log", mode="a"),
        ],
    )
...
def run_pipeline(ingest_only=False, report_only=False):
    ...
    logger.info(f"   Report: reports/weekly_report.html")
    logger.info("=" * 60)
...
<p>Add CSV exports from your social media platforms to the <code>data/</code> folders and push to trigger the pipeline.</p>
<p>See <code>README.md</code> for export instructions.</p>
</body></html>"""
    (REPORTS_DIR / "weekly_report.html").write_text(html)
